...
Holding the model fixed, changing v1 -> v2 changes 42 of 1000 decisions. The EXCLUDE count
rises by 30, coming mostly from **clearing a noisy human-review pile** (28 REVIEW -> EXCLUDE):
engineering, animal-lab, and statistical-methods papers, reports/commentaries, non-registered
hazards, and records where "flooding"/"cold" referred to a therapy or medical procedure rather
than a climate hazard. v2 is **not** simply stricter - it also moves 3 records EXCLUDE -> REVIEW
to protect recall (including a PTSD study after an industrial fire, and a study that does
measure mental health). Only two changes look like genuine errors: one likely **false negative**
(a Typhoon Haiyan resilience / life-satisfaction study, `D0012580`) and one minor **false
positive** (an ML systematic review, `D0054409`); everything else is correct or defensible. A
false negative seen earlier under gpt-4o-mini (`D0020934`, hurricane x adolescent mental health)
is retained here on DeepSeek, showing that drop was a model effect, not a prompt effect. See
`comparison_v1_vs_v2_deepseek_1000.md` for every changed record with an abstract-based assessment.
