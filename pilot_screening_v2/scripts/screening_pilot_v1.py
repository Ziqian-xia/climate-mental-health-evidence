...
VERSION_TAG = "v1"      # version tag; written into the output filenames (v1 / v2)
MODEL = "gpt-4o-mini"        # OpenAI model. Cheap. If HUMAN_REVIEW stays high, try "gpt-4o".
MAX_TOKENS = 800
PAUSE_SEC = 0.2
...
PROMPT_FILES = {
    "router":      "prompts/title_abstract_screening/00_candidate_topics_prompt.md",
    "temperature": "prompts/title_abstract_screening/01_temperature_prompt.md",
    "wildfire":    "prompts/title_abstract_screening/02_wildfire_prompt.md",
    "flood":       "prompts/title_abstract_screening/03_flood_prompt.md",
    "cyclone":     "prompts/title_abstract_screening/04_cyclone_prompt.md",
    "drought":     "prompts/title_abstract_screening/05_drought_prompt.md",
}
...
def get_api_key():
    ...
    log("Paste your OpenAI API key (starts with sk-; it will NOT be shown as you type):")
...
def verify_api_key(client):
    ...
    try:
        ...
        if "401" in msg or "invalid_api_key" in msg or "Incorrect API key" in msg or "Unauthorized" in msg:
            ...
            die("The API key works but has insufficient quota/credit. Add credit at platform.openai.com -> Billing.\nOriginal error: " + msg[:200])
        else:
...
def main():
    log("=" * 64)
    log(" Climate hazards x Mental health -- T/A screening: GPT pilot (v1 prompts, from GitHub)")
    log("=" * 64)
    ...
    out_df.to_csv(f"screening_pilot_results_seed{RANDOM_SEED}_{VERSION_TAG}.csv", index=False, encoding="utf-8-sig")
    review_mask = out_df["final_decision"].str.contains("REVIEW|INCLUDE|ERROR", case=False, na=False)
    out_df[review_mask].to_csv(f"screening_pilot_to_review_seed{RANDOM_SEED}_{VERSION_TAG}.csv", index=False, encoding="utf-8-sig")
    ...
    log(f"\n  Output files: screening_pilot_results_seed{RANDOM_SEED}_{VERSION_TAG}.csv , screening_pilot_to_review_seed{RANDOM_SEED}_{VERSION_TAG}.csv")
    log("=" * 64 + "\n")
