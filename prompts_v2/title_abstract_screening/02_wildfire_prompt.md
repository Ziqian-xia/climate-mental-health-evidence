...
- measured subjective wellbeing, life satisfaction, quality of life, affect, psychological resilience,
  coping, or disaster-related resilience among people exposed to wildfire/fire-smoke events; if the
  abstract is incomplete, use `INCLUDE` with `review_flag = true`, do not EXCLUDE;
...
the abstract discusses wildfire and mental-health outcomes from included studies.
...
eligible mental-health/wellbeing outcome after wildfire exposure. `EXCLUDE` as `non_original` when
it is a review of machine-learning tools, a survey of algorithms, a benchmark of published studies,
or a methods paper with no original hazard-exposed human mental-health data.
