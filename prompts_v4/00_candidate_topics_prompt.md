...
> **Version: v4** — no rule changes in this router. The v4 laboratory / climate-chamber exclusion is
> applied in `01_temperature_prompt.md`, not here, so that such records are excluded *with a logged
> `wrong_exposure` code* rather than dropped silently at routing. Keep assigning `temperature` to
> chamber/laboratory records; the temperature module will exclude them. Only the criteria filename
> reference below has been updated. See `README.md`.
