...
> **Version: v4** — supersedes v3. Two narrow changes, both listed in `prompts_v4/README.md`:
> (a) an explicit **setting requirement** on the temperature hazard — laboratory / climate-chamber /
> engineered-indoor thermal exposure is not an eligible X (new text under *Definitions* and under
> `wrong_exposure`); (b) the `wrong_design` code, already present in this file's output schema, is now
> also present in the `exclusion_code` enum of the five hazard prompts, which had omitted it, whose
> hazard-gate code is also renamed `wrong_hazard` -> `wrong_exposure` to match the name this file has
> always used. No other rule changed.

> **Version: v3** — supersedes v2.1. Adds *Design discipline (v3)*: an eligible study must exploit
> **within-unit variation** in hazard exposure. Study design is now screened at title/abstract when the
> abstract states it clearly; when the design is unstated or ambiguous, route to MAYBE. The *Screening
> philosophy*, *Gate sequence*, *Exclude*, and *Output schema* sections have been updated accordingly
> (new exclusion code `wrong_design`). All other v2.1 content is unchanged.


Criteria version: v4 (DRAFT — pending project review)
...
time-series / quasi-experimental designs and abstracts frequently omit the design.

From v3, design is screened here **only when the abstract makes it explicit**. Many records state
their design plainly ("cross-sectional survey", "qualitative interviews", "case report"); those are
excluded at this stage under *Design discipline (v3)*. Where the design is unstated or ambiguous,
the earlier rule still holds: **do not EXCLUDE a record merely because it does not state an
eligible study design, sample, exposure metric, or effect estimate** — route to MAYBE and let full
text decide. At this stage we gate on the three things an abstract reliably reveals: (1) a relevant
climate hazard exposure, (2) a relevant mental-health/wellbeing outcome, and (3) an explicitly
stated study design. When in doubt, route to MAYBE.
...
- **X = eligible climate hazard exposure**, one of five modules:
  - X1 Temperature: ambient heat OR cold, heatwave, cold spell/snap, heat index, Wet Bulb Globe
    ...
    **Setting requirement (v4):** the exposure must be a *naturally occurring* environmental condition.
    Laboratory or climate/environmental-chamber exposure, cold-pressor and water-immersion protocols,
    experimentally applied thermal stimuli, refrigerated or artificially heated workplaces (cold-storage
    depots, freezer rooms), sauna, cryotherapy and induced hypothermia are NOT eligible X — even when the
    abstract describes the chamber temperature as "ambient", and even when the design is otherwise
    eligible. Outdoor workers and indoor exposure to outdoor-origin heat or cold REMAIN eligible: the
    distinction is machine-generated vs naturally occurring, not occupational vs general population.
    If the abstract does not state the setting, route to MAYBE — never exclude on silence.
...
G4 Empirical study (not review/   -> NO: EXCLUDE (non_original)
   ...
   |
G5 Design exploits within-unit    -> NO (design stated and ineligible): EXCLUDE (wrong_design)
   variation (panel/time-series/      UNCLEAR or UNSTATED: MAYBE
   case-crossover/DiD)?
   |
=> INCLUDE
```
...
Design (Z) IS a gate from v3, but only against an explicitly stated design (see *Design discipline
(v3)*); an unstated design routes to MAYBE, never EXCLUDE. Risk of bias, exposure-metric
objectivity, comparator, and presence of a quantitative effect estimate remain NOT gates at
title/abstract. They are recorded at full text.
...
2. `wrong_exposure` — no eligible climate hazard X at all (e.g. earthquakes, volcanic eruptions,
   ...
   ALSO (v4): thermal exposure staged in a laboratory or climate/environmental chamber, or produced by
   an engineered indoor environment (see the *Setting requirement* under X1). This is a hazard-gate
   failure, not a design failure — use `wrong_exposure`, never `wrong_design`.
...
6. `wrong_design` — the abstract explicitly states a design that cannot identify a within-unit effect
   (cross-sectional or single-wave survey, qualitative/interview study, case report or case series,
   ecological correlation with no within-unit time variation, or an intervention/treatment-efficacy
   trial). See *Design discipline (v3)*. If the design is not stated, use MAYBE, not EXCLUDE.

Do NOT exclude for: **unstated or ambiguous study design** (route to MAYBE); self-reported outcomes
where measurement is or may be repeated (sorted at full text); missing effect estimate; non-English
language; preprint status (medRxiv/SSRN preprints are eligible); old publication date (no date
restriction).
...
## Design discipline (v3)

An eligible study must be able to identify the effect of the hazard by comparing the **same unit**
(person, household, small area, or facility) **across time**, i.e. it must exploit **within-unit
variation** in hazard exposure. Between-group contrasts measured once do not identify the effect and
are out of scope.

This is gate **G5**, applied at title/abstract. It replaces the earlier rule that design is decided
only at full text. It bites ONLY against a design the abstract states explicitly: if the record
plainly says it is cross-sectional, qualitative, a case report, or an intervention trial, exclude it
here; if the design is unstated or ambiguous, route to MAYBE (see *When the design is not stated*).

### Eligible designs (within-unit variation present)

- Longitudinal panel or cohort with **repeated measures on the same individuals**, where a hazard
...
- Time-series or interrupted time-series on the same unit (daily/weekly counts for a population,
  city, or facility);
- Case-crossover and time-stratified case-crossover (each case serves as its own control);
- Difference-in-differences, event-study, fixed-effects panel, and other quasi-experimental designs
  that compare the same units before and after exposure;
- Regression discontinuity or instrumental-variable designs built on the same units over time.

**Self-reported outcomes are eligible** provided the measurement is repeated: the same person reports
mental health on several occasions and the hazard strikes between measurements, so pre- and
post-hazard values can be compared within that person.

### Ineligible designs (no within-unit variation)

`EXCLUDE` with code `wrong_design` when the abstract makes clear that the study is:
...
- **one-wave** — a single post-hazard assessment compared against a separate, unexposed sample, or
  against population norms, rather than against the same units before exposure;
...
- **case report or case series** — clinical vignettes, single-patient or small-N descriptive accounts;
- **ecological correlation without a time dimension within units** — e.g. season-of-birth or
  cross-country correlations with no within-unit exposure variation;
- **intervention or treatment-efficacy studies** — trials of therapies, counselling, or digital tools
  after a hazard, which estimate the effect of the treatment rather than of the hazard.

### When the design is not stated

Abstracts frequently omit the design, and excluding on silence would cost recall. Therefore:

- If the abstract does **not** state the design, or states it ambiguously (e.g. "prospective" without
  the number of waves), route to **MAYBE** for full-text resolution. Do NOT exclude.
- If the record has **no abstract**, judge on title alone and route to **MAYBE**, per the existing
  MAYBE rule.
- Repeated post-hazard follow-ups **without any pre-hazard baseline** (e.g. assessments at 6 months,
  2 years, and 7 years after the event) are a **boundary case**: they contain within-person repeated
  measures but no pre-exposure comparison. Route to **MAYBE** and flag for project review.

### Interaction with the other gates

Design discipline applies **after** hazard (X) and outcome (Y) are satisfied. A record that fails the
hazard or outcome gate is excluded with its own code; `wrong_design` is used only for records that are
on-topic but whose stated design cannot identify a within-unit effect.
